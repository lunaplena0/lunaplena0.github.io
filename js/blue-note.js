...
// [추가] 시:분:초 또는 분:초 문자열을 초(숫자)로 변환하는 함수
        ...
        groupDiv.innerHTML = `
            ...
            <div class="cry-list">
                ${groups[month].map(cry => {
                    ...
                    // --- [변경 핵심 구간: 시간 -> 초 파라미터 조립] ---
                    let finalLink = cry.link;
                    if (hasLink) {
                        const totalSeconds = timeToSeconds(cry.time);
                        if (totalSeconds !== null) {
                            finalLink = `${cry.link}?change_second=${totalSeconds}`;
                        }
                    }
                    // --------------------------------------------------

                    const linkAttr = hasLink ? `href="${finalLink}" target="_blank" rel="noopener noreferrer"` : `href="javascript:void(0)"`;
