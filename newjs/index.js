...
const CONFIG_URL = TSV_URL; // 동일한 설정 주소
...
// 2. 메인 데이터 초기화 (프로필, 방송 상태, 공지, VOD, 주간 일정)
async function init() {
    const overlay = document.getElementById('loading-overlay');
    try {
...
function updateTimelineDisplay(expanded) {
    ...
    const displayData = expanded ? allTimelineData : allTimelineData.slice(0, INITIAL_SHOW);
...
// 11. 통합 DOMContentLoaded 이벤트 (페이지 로딩 시 한 번만 실행)
