...
let currentMainTag = null;
   ...
   const catColors = { 
    // [고정] 필수 지정 색상
    '구독+': '#f1c40f',  // 후원자 전용 혜택을 상징하는 [선플라워 골드]
    '19': '#ff4757',    // 직관적인 경고와 제한을 의미하는 [강렬한 레드]

    // [조정] 콘텐츠 카테고리 (색상 중복 방지 최적화)
    '노래': '#6c5ce7',   // 게임/시네티와 확실히 차별화되는 [진한 딥 퍼플]
    '시네티': '#00d8ff',  // 영화 감상의 청량함을 담은 [밝은 네온 사이언]
    '게임': '#341f97',   // 시네티와 겹치지 않도록 아주 깊고 진한 [네이비 블루]
    '소통': '#2ed573',   // 편안한 대화 시간을 상징하는 [프레시 그린]
    'ASMR': '#fda7df',   // 보라색(노래)과 겹치지 않게 변경한 [밝은 라벤더 핑크]
    '풀트': '#ff9f43',   // 역동적인 신체 퍼포먼스를 상징하는 [비비드 오렌지]
    '대회': '#1dd1a1',   // 레드/그린과 구분되는 승리의 [와일드 민트]
    '합방': '#ff7675',   // 오렌지(풀트)와 겹치지 않는 화사한 [코랄 핑크]
